...
# Final features list
features_columns = [col for col in list(train_df)
                    if col not in rm_cols + features_discard]
...
if not LOCAL_TEST:
    ...
    test_predictions[['TransactionID', 'isFraud']].to_csv('kernel1_remodel/submission.csv', index=False)

# %%
# 特徴作るならカテゴリごとのtimedeltaは使った方がいいかも、前もそうだしあとも
## LOCAL_TEST = True
# max auc→0.9171410539971948
# feat_imp
# 473    308                 uid_fq_enc
# 474    316   uid2_TransactionAmt_mean
# 475    324                         D8
# 476    389                uid2_fq_enc
# 477    396                        C13
# 478    408                      id_20
# 479    413                         D4
# 480    418       P_emaildomain_fq_enc
# 481    421                         D2
# 482    422                      id_02
# 483    423                        D10
# 484    453   card2_TransactionAmt_std
# 485    477                      dist1
# 486    516               card1_fq_enc
# 487    551   card1_TransactionAmt_std
# 488    604                        D15
# 489    604               card2_fq_enc
# 490    621  card2_TransactionAmt_mean
# 491    655               addr1_fq_enc
# 492    673                   uid_DT_M
# 493    705                   uid_DT_W
# 494    713  card1_TransactionAmt_mean
# 495    763                      card2
# 496    808                      addr1
# 497    819    uid3_TransactionAmt_std
# 498    879                uid3_fq_enc
# 499    908                   uid_DT_D
# 500    951   uid3_TransactionAmt_mean
# 501   1181                      card1
# 502   1210             TransactionAmt

## LOCAL_TEST = False
# max auc→こっちの方が普通のtarget encodeより精度良さげ