...
# temp_df = temp_df.sort_values(['DT'])[['uid', 'uid2', 'uid3', 'DT', 'DT_M', 'DT_W', 
#                                        'DT_D', 'DT_hour', 'DT_day_week', 'DT_day',
#                                        'TransactionAmt', 'isFraud']]
# for agg_col in ['uid', 'uid2', 'uid3']:
#     new_col_name = 'shifted_' + agg_col + '_TransactionAmt'
#     temp_df[new_col_name] = temp_df.groupby(agg_col)['TransactionAmt'].shift()
#     new_col_name = 'shifted_opposite_' + agg_col + '_TransactionAmt'
#     temp_df[new_col_name] = temp_df.groupby(agg_col)['TransactionAmt'].shift(-1)
...
# for agg_col in ['uid', 'uid2', 'uid3']:
#     new_col_name = 'shifted_' + agg_col + '_TransactionAmt'
#     train_df[new_col_name] = train_df.groupby(agg_col)['TransactionAmt'].shift()
#     test_df[new_col_name] = train_df.groupby(agg_col)['TransactionAmt'].shift()
#     train_df['diff_'+new_col_name] = train_df['TransactionAmt'] - train_df[new_col_name]
#     test_df['diff_'+new_col_name] = test_df['TransactionAmt'] - test_df[new_col_name]
#     new_col_name = 'shifted_opposite_' + agg_col + '_TransactionAmt'
#     train_df[new_col_name] = train_df.groupby(agg_col)['TransactionAmt'].shift(-1)
#     test_df[new_col_name] = test_df.groupby(agg_col)['TransactionAmt'].shift(-1)
#     train_df['diff_'+new_col_name] = train_df['TransactionAmt'] - train_df[new_col_name]
#     test_df['diff_'+new_col_name] = test_df['TransactionAmt'] - test_df[new_col_name]
...
train_df['isTrain'] = 1
test_df['isTrain'] = 0
concatted_df = pd.concat([train_df, test_df], axis=0)

for agg_col in ['uid', 'uid2', 'uid3']:
    new_col_name = 'shifted_' + agg_col + '_TransactionAmt'
    concatted_df[new_col_name] = concatted_df.groupby(agg_col)['TransactionAmt'].shift()
    concatted_df['diff_'+new_col_name] = concatted_df['TransactionAmt'] - concatted_df[new_col_name]
    new_col_name = 'shifted_opposite_' + agg_col + '_TransactionAmt'
    concatted_df[new_col_name] = concatted_df.groupby(agg_col)['TransactionAmt'].shift(-1)
    concatted_df['diff_'+new_col_name] = concatted_df['TransactionAmt'] - concatted_df[new_col_name]

train_df = concatted_df[concatted_df['isTrain'] == 1]
test_df = concatted_df[concatted_df['isTrain'] == 0]

# %%
# tr_temp_df = train_df.copy()
# te_temp_df = test_df.copy()
# tr_temp_df = tr_temp_df.sort_values(['DT'])[['uid', 'uid2', 'uid3', 'DT', 'DT_M', 'DT_W', 
#                                              'DT_D', 'DT_hour', 'DT_day_week', 'DT_day',
#                                              'TransactionAmt', 'isFraud']]
# te_temp_df = tr_temp_df.sort_values(['DT'])[['uid', 'uid2', 'uid3', 'DT', 'DT_M', 'DT_W', 
#                                              'DT_D', 'DT_hour', 'DT_day_week', 'DT_day',
#                                              'TransactionAmt', 'isFraud']]
# tr_temp_df['isTrain'] = 1
# te_temp_df['isTrain'] = 0

# %%
# agg_df = temp_df.groupby(['uid', 'DT_D']).agg('mean').reset_index()
...
if not LOCAL_TEST:
    ...
    test_predictions[['TransactionID','isFraud']].to_csv('kernel1_remodel6/submission.csv', index=False)

# %%
## LOCAL_TEST = True
# auc→0.9215778099668036(ver5+lag_featureを追加し差分を計算、lag_featを削除))(今までのmax→0.9211773468405661)更新！

# feat_imp
# 537   1556                           card2_id_02_mean
# 538   1562                              card1_D15_std
# 539   1572                                         D2
# 540   1615                    uid3_diff_is_fraud_mean
# 541   1651                                        C13
# 542   1696                                      card2
# 543   1716   diff_shifted_opposite_uid_TransactionAmt
# 544   1728                                      id_02
# 545   1748                                   uid_DT_M
# 546   1770                                      dist1
# 547   1781                uid3_diff_is_not_fraud_mean
# 548   1795                           card1_id_02_mean
# 549   1846                          diff_is_not_fraud
# 550   1857                            card1_id_02_std
# 551   1875                   uid3_TransactionAmt_mean
# 552   1976                               addr1_fq_enc
# 553   1981            diff_shifted_uid_TransactionAmt
# 554   2036                                        D15
# 555   2165                                   uid_DT_W
# 556   2185                                uid3_fq_enc
# 557   2537                                      addr1
# 558   2621                                   uid_DT_D
# 559   2669                             TransactionAmt
# 560   2713                              uid3_D15_mean
# 561   2812                               uid3_D15_std
# 562   2863           diff_shifted_uid3_TransactionAmt
# 563   2875  diff_shifted_opposite_uid3_TransactionAmt
# 564   3111                                      card1
# 565   3228                             uid3_id_02_std
# 566   3377                            uid3_id_02_mean
# 
## LOCAL_TEST = False
# max auc(lb)→0.9485(feature_eliminateした場合)
# max auc(lb)→0.9488(feature_eliminateしてない場合)
# max auc(cvの)→0.976896

# %%
# もう少しやりたいこと
# uid + DorMorWで平均をとってからラグを加える
