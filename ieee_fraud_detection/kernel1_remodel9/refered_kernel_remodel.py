...
# %%
# 大事そうなカテゴリデータを10個以内にリストアップ
cate_cols = ['ProductCD', 'M4', 'card4', 'card5', 'uid', 'uid2', 'uid3']
temp_df = pd.concat([train_df[cate_cols], test_df[cate_cols]])
for row_col in cate_cols:
    for col_col in cate_cols:
        if row_col != col_col:
            sentence_dict = {}
            for p in temp_df[row_col].unique():
                temp_list = list(map(str, list(temp_df[temp_df[row_col] == p][col_col])))
                temp_sentence = ' '.join(temp_list)
                sentence_dict[p] = temp_sentence
            sentence_df = pd.DataFrame(sentence_dict.items(), columns=[row_col, 'sentence'])
            counter = CountVectorizer()
            X = counter.fit_transform(sentence_df['sentence'])
            topic_num = 3
            lda = LatentDirichletAllocation(n_components=topic_num, random_state=SEED)
            new_col_name = [row_col + '_' + col_col + '_topic%d'%(i+1)
                            for i in range(topic_num)]
            topics_df = pd.DataFrame(lda.fit_transform(X), columns=new_col_name)
            topics_df = pd.concat([sentence_df, topics_df], axis=1).drop('sentence', axis=1)
            train_df = pd.merge(train_df, topics_df, how='left', on=row_col)
            test_df = pd.merge(test_df, topics_df, how='left', on=row_col)
